...
public class GetAllDealerMoney  implements Command<String, IoSession>{
	private static final Logger logger = LoggerFactory.getLogger(GetAllDealerMoney.class);
	
	@Override
	public void execute(MXchangerService<String> service, IoSession session, String data) {
		String answer;
		try {
			answer = service.getAllDealerMoney(data);
